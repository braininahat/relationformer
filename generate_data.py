...
def patch_extract(save_path, image, seg, mesh, device=None):
    ...
    for i, start in enumerate(list(np.array(ind).reshape(3, -1).T)):
        ...
        for patch, patch_seg, patch_coord, patch_edge in zip(
            ...
            if patch_seg.sum() > 10:
                save_input(
                    save_path, image_id, patch, patch_seg, patch_coord, patch_edge
                )
                image_id = image_id + 1
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = []
        for idx, seg_file in tqdm(enumerate(seg_files[:40]), total=len(seg_files[:40])):
            futures.append(executor.submit(process_data, idx, seg_file))
    ...
    for idx, seg_file in tqdm(enumerate(seg_files[40:50]), total=len(seg_files[40:50])):
        image_data, _ = load(raw_files[idx])
        image_data = np.int32(image_data)
        seg_data, _ = load(seg_files[idx])
        seg_data = np.int8(seg_data)
